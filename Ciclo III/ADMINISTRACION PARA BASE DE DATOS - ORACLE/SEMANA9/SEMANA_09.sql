...
----------------------------------------------------------------------------------------------------------
-- CASOS BASADOS EN PROCEDIMIENOS ALMACENADOS
----------------------------------------------------------------------------------------------------------
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE ('POTENCIA       : ' || TO_CHAR(POWER(X,Y)));
   DBMS_OUTPUT.PUT_LINE ('FRACCION X     : ' || TO_CHAR(1/X));
   DBMS_OUTPUT.PUT_LINE ('FRACCION Y     : ' || TO_CHAR(1/Y));
   DBMS_OUTPUT.PUT_LINE ('RAIZ CUADRADA X: ' || TO_CHAR(SQRT(X)));
   DBMS_OUTPUT.PUT_LINE ('RAIZ CUADRADA Y: ' || TO_CHAR(SQRT(Y)));
   DBMS_OUTPUT.PUT_LINE ('==================================');      
END;
...
-- EJEMPLO CASUAL:
-- Elaborar un Procedmmiento almacenado, el cual pida por parámetro
-- la edad de una persona, el procedimientos debe imprimir
-- si dicha persona es mayor  o menor de edad.

CREATE OR REPLACE PROCEDURE SP_CUIDADANO (EDAD NUMBER)
IS
BEGIN
      IF EDAD >= 18 THEN
...
SET SERVEROUTPUT ON;
DECLARE
    B NUMBER; H NUMBER; AREA NUMBER:=0;
BEGIN
    B := &B;
    H := &H;
...
--y estas son: LPROG, ANALISIS, ARTE, LIDERAZGO, CAD, PROYECTOS.

--ANALIZAR: MOSTRANDO LOS DATOS DE LA TABLA LIBRO.
SELECT * FROM LIBRO;

--CREANDO EL PROCEDIMIENTO ALMACENADO SP_FILTRO_AREA_LIBRO
CREATE OR REPLACE PROCEDURE SP_FILTRO_AREA_LIBRO
     (AREA IN VARCHAR2)
IS
BEGIN
    DECLARE CURSOR FILTRO_1 IS (SELECT * FROM LIBRO WHERE AREALIB=AREA);
    A  LIBRO.CODLIB%TYPE;
    ...
    BEGIN
        ...
        LOOP
            DBMS_OUTPUT.PUT_LINE ('---------------------------------------');
            DBMS_OUTPUT.PUT_LINE ('Datos de los Libros:');
            DBMS_OUTPUT.PUT_LINE ('---------------------------------------');
            DBMS_OUTPUT.PUT_LINE ('Codigo          : ' || A);
            DBMS_OUTPUT.PUT_LINE ('Titulo          : ' || B);
            DBMS_OUTPUT.PUT_LINE ('Autor           : ' || C);
            DBMS_OUTPUT.PUT_LINE ('Editorial       : ' || D);
            DBMS_OUTPUT.PUT_LINE ('Area            : ' || E);
            DBMS_OUTPUT.PUT_LINE ('Precio          : ' || F);
            DBMS_OUTPUT.PUT_LINE ('Nro Hojas       : ' || G);
            DBMS_OUTPUT.PUT_LINE ('año del libro   : ' || H);
            DBMS_OUTPUT.PUT_LINE ('');
            FETCH FILTRO_1 INTO A, B, C, D, E, F, G, H;
        END LOOP;
        CLOSE FILTRO_1;
...
SET SERVEROUT ON;
BEGIN
        --SP_FILTRO_AREA_LIBRO ('LPROG');
        --SP_FILTRO_AREA_LIBRO ('ANALISIS');
        --SP_FILTRO_AREA_LIBRO ('ARTE');
        --SP_FILTRO_AREA_LIBRO ('LIDERAZGO');
        --SP_FILTRO_AREA_LIBRO ('CAD');
        SP_FILTRO_AREA_LIBRO ('PROYECTOS');
END;
...
IS
BEGIN
      DECLARE CURSOR FILTRO_2 IS (SELECT * FROM TRABAJADOR2
                           WHERE COD_TRA = CODIGO);
      A TRABAJADOR2.COD_TRA%TYPE;
      B TRABAJADOR2.AP_TRA%TYPE;
      C TRABAJADOR2.AM_TRA%TYPE;
      D TRABAJADOR2.NOM_TRA%TYPE;
      E TRABAJADOR2.FECHA_NA%TYPE;
      F TRABAJADOR2.SUELDO%TYPE;
      G TRABAJADOR2.GENERO_TRA%TYPE;
      H TRABAJADOR2.AREA_TRA%TYPE;
      BEGIN
          OPEN FILTRO_2;
          ...
          LOOP
             IF F IS NULL THEN
                UPDATE TRABAJADOR2 SET SUELDO=1000 WHERE COD_TRA=A;
                COMMIT;
             END IF;
             FETCH FILTRO_2 INTO A, B, C, D, E, F, G, H;
          END LOOP;
...
SELECT * FROM TRABAJADOR2 WHERE SUELDO IS NULL;
...
BEGIN
      SP_ACTUALIZA_SUELDO ('00011');
      DBMS_OUTPUT.PUT_LINE('SUELDO ACTUALIZADO');
END;

--VERIFIACNDO EL CAMBIO DE SUELDO DEL CODIGO '00011'
SELECT * FROM TRABAJADOR2;
SELECT * FROM TRABAJADOR2 WHERE SUELDO IS NULL;
