...
-- EJERCICIO BASADO EN LA SESION 6

-- Ejemplo 1:
-- La tabla STAFF posee elementos nulos en la columna COMM (comisiones),
-- crear un cursor de actualización el cual modifique estos
-- elementos nulos por cero.

-- Consulta para saber que comisiones son nulas obtendremos:

SELECT * FROM STAFF WHERE COMM IS NULL; 

--SOLUCION --> USANDO CURSOR EXPLICITO --> TIENE VARUAS FILAS DE DATOS
...
BEGIN
     ...
     WHiLE ASIGNAR_CEROS%Found ---> vvvvvvvvvvvF
     LOOP 
...
SELECT * FROM STAFF WHERE COMM = 0; 




----------------------------------------------------------------------------------------------------------
-- CASOS BASADOS EN CURSORES IMPLICITOS - EXPLICITOS
----------------------------------------------------------------------------------------------------------

-- Ejercicio 1:
-- Se pide crear un cursor que sea capaz de mostrar los pedidos realizados, 
-- según la tabla CABECERA_PEDIDO, esta tabla tiene 3 pedidos,
-- estos son: ‘00001’, ‘00002’ y ‘00003’, para ello se debe
-- ingresar el número del pedido, por intermedio de variables
-- de sustitución &, y brindar la información solicitada, 
-- tal como se muestran en las siguientes imágenes.
...
DECLARE
    ...
    -- CURSOR EXPLICITO, GENERA EL DETALLE DE PEDIDOS
...
BEGIN
    ...
    --ESTOS SELECT BUSCAN LOS NOMBRES DE LOS CLIENTES y EMPLEADOS
    ...
    WHiLE DETALLE%Found --> Si se encontró una fila de datos (v) o (f)
    LOOP
...
-- EJERCICIO 2:
-- Se pide crear un cursor, que sea capaz de mostrar datos de las facturas
-- que han sido realizadas como PAGADAS, según la tabla COMPROBANTE, esta
-- tiene 3 documentos (1 factura y 2 boletas)
-- estas son: ‘F0001’, ‘B0001’ y ‘B0002’, tal como se muestra
...
DECLARE
    ...
    NombreEmp     EMPLEADO.NOMBRE%TYPE;
    NombreProd    PRODUCTO.NOMBRE%TYPE;
    CantProd      DETALLE_PEDIDO.CANT%TYPE; PreProd  PRODUCTO.PRECIO%TYPE;
    ImpCompra     NUMBER(10,2); TOTAL_IMPORTE NUMBER(10,2):=0;
    IGV NUMBER(10,2):=0; TOTAL_GENERAL NUMBER(10,2):=0;
    CURSOR DETALLE IS (SELECT D.ITEM, P.NOMBRE, D.CANT,
                       P.PRECIO, (D.CANT* P.PRECIO) IMPORTE
                       ...
                       WHERE D.IDPROD = P.IDPROD AND
                             ...
                             C.ESTADO ='PAGADO');
BEGIN
    SELECT PEDIDO.IDPEDIDO, PEDIDO.IDCLI,
           PEDIDO.IDEMP,COMPROBANTE.ESTADO
    INTO CodigoP, CodigoC, CodigoE, Est 
    FROM PEDIDO, COMPROBANTE
    WHERE PEDIDO.IDPEDIDO = COMPROBANTE.IDPEDIDO AND
