...
-- de sustitución "&" el código del libro, por ejemplo ‘1000’, los datos de dicho 
...
BEGIN
    ...
    --> Si en caso la consulta devueve 0 registros
        --> se genera una excepción llamada NO_DATA_FOUND(No encontro datos)
        --> ya no continua la ejecución, salta a la linea de EXCEPTION
    --> Si en caso la consulta devulve 1(deseado) o más REGISTROS,
...
-- EJEMPLO 2:
-- Se desea vender todos los libros de la librería, clasificados por área del
-- libro (ANALISIS, ARTE, PROYECTOS, LPROG, LIDEREZGO y CAD), genere un código 
-- PL/SQL que realice dicha acción, con totalizaciones incluidas.  Redacte el
-- código desde el editor de Oracle PL/SQL.
SELECT * FROM LIBRO;
...
-- crear un cursor de actualización el cual modifique estos elementos
-- nulos por cero.
...
SELECT * FROM STAFF;
-- EJEMPLO 2:
-- En la tabla STAFF, existen años nulos en la columna YEARS, se pide crear
...
-- Tercero, Creamos el cursor explícito de Inserción de datos,
...
-- Después de la ejecución del código, verificamos en la tabla
...
-- Como ya sabemos quiénes son, procedemos a crear el cursor de actualización
